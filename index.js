...
async function fetchStatus(address) {
  const result = await fetch(
    `https://api.tzkt.io/v1/contracts/${IDENTITY_ADDRESS}/bigmaps/users/keys?key.eq=${address}`
  )
...
client.on("messageCreate", async (message) => {
  ...
  const fetchPrereqs = ["!verify", "!status"]
  ...
  if (fetchPrereqs.includes(message.content)) {
    try {
      ...
      return message.reply({
        // Error during tzprofiles fetch
        content:
          "Something went wrong while checking your profile. Please try again later. (Fetch Error)",
      })
    }
  ...
  switch (message.content) {
    case "!status":
      if (profile.data.tzprofiles.length === 0) {
        ...
        return message.reply({
          content:
            "You have not linked your profile to a Tezos account. Please do so by following the instructions on <https://tzprofiles.com>.",
        })
      }
    ...
    case "!help":
      let embed = new Discord.MessageEmbed()
        ...
        .setDescription(
          "This bot is used to do cool stuff on this Discord server, it can link your versum profile, provide help, and quickly link information."
        )
        .addFields(
          {
            name: "!status",
